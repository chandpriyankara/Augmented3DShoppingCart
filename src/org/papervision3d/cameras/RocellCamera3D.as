package org.papervision3d.cameras
{
	...
	// this was initially written by papervision and was edited as was needed for the Rocell 
	//Virtual Maket Place by FortunaGlobal (pvt) Ltd
	//www.fortunaglobal.com
	//author ChadPriyankara@engineering.com
	/**
	 * <p>
	 ...
	 * @author ChadPriyankara@engineering.com
	 */
	public class RocellCamera3D extends Camera3D
	{
		/** @private */
		...
		/** @public */
		public var displayPropertieWindow:Boolean = true;
		/** @public */
		public var mouseNavigation:Boolean = true;
		/** @private */
		 ...
		 */
		public function RocellCamera3D(viewport3D:Viewport3D, fovY:Number = 90, near:Number = 10, far:Number = 5000) 
		{
		...
		protected function displayProperties():void 
		{
			...
			if(displayPropertieWindow==true){viewport3D.addChild(_propertiesDisplay);}
		}
		...
		protected function setupEvents():void 
		{
			...
			viewportStage.addEventListener(MouseEvent.MOUSE_WHEEL,cameraHeight);
		...
		{
			//if(mouseNavigation == true){mouseDownHandler(event);mouseNavigation = false;}
			//if(startRotationY == null){startRotationY = 0;}
			//if(startRotationX == null){startRotationX = 0;}
			//trace("mouse moving...startRotationY:"+startRotationY+"  startRotationX:"+startRotationX);
			//trace("mouse moving...viewportStage.mouseX:"+viewportStage.mouseX+"  viewportStage.mouseY:"+viewportStage.mouseY);
			if(mouseNavigation)
			{
				//startPoint = new Point(viewportStage.mouseX, viewportStage.mouseY);
				//startRotationY = this.rotationY;
				//startRotationX = this.rotationX;
				targetRotationY = startRotationY - (startPoint.x - viewportStage.mouseX) / 4;
				targetRotationX = startRotationX + (startPoint.y - viewportStage.mouseY) / 4;
				
				trace("targetRotationY:" + targetRotationY + "\ntargetRotationX:" + targetRotationX);
			}
			//trace("...targetRotationY:"+targetRotationY+"   targetRotationX:"+targetRotationX);
		}
		...
		public function cameraHeight(event:MouseEvent):void
		{
			// this function will move the camera view up and down according the rotation of the mouse wheel
			// the rate of mouse changin can be set by changing the value of 'rate'
			rate = 10;
			if(mouseNavigation)
			{
				this.y+=(event.delta*rate);
			}
		}
		...
		public override function translate( distance:Number, axis:Number3D ):void
		{
			...
			//this.y += distance * vector.y;//changed by ChadPriyankara@engineering.com to make navigation easier(restored badk to original)
			this.z += distance * vector.z;
		}
		 
		public override function moveForward  ( distance:Number ):void 
		{ 
